            ...
            match node_data {
                NodeData::Schema { title, properties, .. } => {
                    if properties.contains_key("source_type") && properties.contains_key("target_type") {
                        // Connection schema
                        let source_type = properties.get("source_type")
                        ...
                        schema_string.push_str(&format!("  Connection Type: {} ({} -> {})\n", 
                            connection_type, source_type, target_type));
                        
                        // Filter out source_type and target_type from properties
                        let filtered_properties: Vec<(&String, &Value)> = properties.iter()
            ...
            for op in &level.operations {
                match op {
                    SelectionOperation::Filter(conditions) => {
                        ...
                        let conditions_str: Vec<String> = conditions.iter()
                    ...
                    SelectionOperation::Sort(sort_fields) => {
                        ...
                        let sorts_str: Vec<String> = sort_fields.iter()
                            .map(|(field, ascending)| 
                    ...
                    SelectionOperation::Traverse { connection_type, direction, max_nodes } => {
        ...
        for (parent, children) in level.iter_groups() {
            match parent {
                Some(parent_idx) => {
                    if let Some(parent_node) = graph.get_node(*parent_idx) {
                        match parent_node {
                            NodeData::Regular { node_type, title, .. }
            ...
            if !children.is_empty() {
                ...
                let child_type = first_child.map(|node| match node {
                    NodeData::Regular { node_type, .. } | NodeData::Schema { node_type, .. } => node_type
                });
                
                let indices: Vec<String> = children.iter()
                    .take(5)
                ...
                output.push_str(&format!("            Children: {} {}{}\n",
                    ...
                    if children.len() <= 5 { format!(": {}", indices_str) } else { format!(", e.g. {}", indices_str) }
                ));
            }
        }
    }
