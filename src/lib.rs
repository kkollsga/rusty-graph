...
use graph::io_operations::{load_file, load_graph_mmap};
...
#[pyfunction]
fn load_mmap(py: Python<'_>, path: String) -> PyResult<KnowledgeGraph> {
    py.detach(|| load_graph_mmap(&path))
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyIOError, _>(e.to_string()))
}
...
fn kglite(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(load_mmap, m)?)?;
