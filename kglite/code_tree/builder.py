...
def _parse_all(src_root: Path, verbose: bool = False) -> tuple[ParseResult, frozenset[str]]:
    ...
    if not parsers:
        raise FileNotFoundError(f"No supported source files found in {src_root}")
...
def _parse_all_roots(
    ...
    for root in source_roots:
        ...
        already_covered = any(root.path != d and root.path.is_relative_to(d) for d in parsed_dirs)
        if already_covered:
            if root.is_test:
        ...
        for parser in parsers:
            ...
            if correct_prefix != parser_prefix:
                for f in result.files:
                    f.module_path = _reprefix(f.module_path, parser_prefix, correct_prefix, sep)
                for fn in result.functions:
                    fn.qualified_name = _reprefix(fn.qualified_name, parser_prefix, correct_prefix, sep)
                for c in result.classes:
                    c.qualified_name = _reprefix(c.qualified_name, parser_prefix, correct_prefix, sep)
                for e in result.enums:
                    e.qualified_name = _reprefix(e.qualified_name, parser_prefix, correct_prefix, sep)
                for i in result.interfaces:
                    i.qualified_name = _reprefix(i.qualified_name, parser_prefix, correct_prefix, sep)
                for a in result.attributes:
                    a.qualified_name = _reprefix(a.qualified_name, parser_prefix, correct_prefix, sep)
                    a.owner_qualified_name = _reprefix(a.owner_qualified_name, parser_prefix, correct_prefix, sep)
                for co in result.constants:
                    co.qualified_name = _reprefix(co.qualified_name, parser_prefix, correct_prefix, sep)
                for tr in result.type_relationships:
                    tr.source_type = _reprefix(tr.source_type, parser_prefix, correct_prefix, sep)
                    if tr.target_type:
                        tr.target_type = _reprefix(tr.target_type, parser_prefix, correct_prefix, sep)
                    for m in tr.methods:
                        m.qualified_name = _reprefix(m.qualified_name, parser_prefix, correct_prefix, sep)
    ...
    if not combined.files:
        raise FileNotFoundError(f"No supported source files found in {project_root}")
...
def _build_call_edges(
    all_functions: list[FunctionInfo], max_targets: int = 5, excluded_names: frozenset[str] = frozenset()
) -> pd.DataFrame:
    ...
    for fn in all_functions:
        for called_name, call_line in fn.calls:
            ...
            if receiver_hint:
                filtered = [t for t in targets if qname_to_owner.get(t) == receiver_hint]
                if filtered:
                    targets = filtered
            ...
            if len(targets) > 1:
                ...
                if caller_prefix:
                    same_owner = [t for t in targets if qname_to_prefix.get(t, "") == caller_prefix]
                    if same_owner:
                        targets = same_owner
            ...
            if len(targets) > 1:
                ...
                same_file = [t for t in targets if qname_to_file.get(t) == caller_file]
                if same_file:
                    targets = same_file
            ...
            if len(targets) > 1:
                ...
                same_lang = [t for t in targets if _infer_lang_group(t) == caller_lang]
                if same_lang:
                    targets = same_lang
    ...
    return pd.DataFrame(edges) if edges else pd.DataFrame(columns=["caller", "callee", "call_lines", "call_count"])
...
def _load_graph(
    ...
    if project_info is not None:
        ...
        graph.add_nodes(data=proj_df, node_type="Project", unique_id_field="name", node_title_field="name")
        ...
        if project_info.dependencies:
            ...
            graph.add_nodes(data=deps_df, node_type="Dependency", unique_id_field="dep_id", node_title_field="name")
    ...
    if result.files:
        ...
        graph.add_nodes(data=files_df, node_type="File", unique_id_field="path", node_title_field="filename")

    if modules:
        modules_df = pd.DataFrame(modules)
        graph.add_nodes(data=modules_df, node_type="Module", unique_id_field="qualified_name", node_title_field="name")
            ...
            [
                {
                    ...
                    **{k: v for k, v in f.metadata.items() if not isinstance(v, (list, dict))},
                }
                for f in result.functions
            ]
        )
        graph.add_nodes(data=funcs_df, node_type="Function", unique_id_field="qualified_name", node_title_field="name")
    ...
    for kind, node_type in [("struct", "Struct"), ("class", "Class")]:
        ...
        if subset:
                ...
                [
                    {
                        ...
                        if c.qualified_name in attrs_by_owner
                        else None,
            ...
            graph.add_nodes(data=df, node_type=node_type, unique_id_field="qualified_name", node_title_field="name")
            ...
            [
                {
                    ...
                    "variant_details": json.dumps(e.variant_details) if e.variant_details else None,
                }
                for e in result.enums
            ]
        )
        graph.add_nodes(data=df, node_type="Enum", unique_id_field="qualified_name", node_title_field="name")

    for kind, node_type in [("trait", "Trait"), ("protocol", "Protocol"), ("interface", "Interface")]:
                ...
                [
                    {
                        ...
                        if i.qualified_name in attrs_by_owner
                        else None,
                    }
                    for i in subset
                ]
    ...
    if result.constants:
        ...
        graph.add_nodes(data=df, node_type="Constant", unique_id_field="qualified_name", node_title_field="name")
        ...
        for target_type in sorted(target_types):
            ...
            if subset:
                ...
                graph.add_connections(
                    ...
                    source_type="File",
                    source_id_field="file_path",
                    target_type=target_type,
                    target_id_field="item_qname",
                )
    ...
    if contains_edges:
        ...
        graph.add_connections(
            data=df,
            connection_type="HAS_SUBMODULE",
            source_type="Module",
            source_id_field="parent",
            target_type="Module",
            target_id_field="child",
        )
    ...
    if len(call_edges_df) > 0:
        graph.add_connections(
            ...
            source_id_field="caller",
            target_type="Function",
            target_id_field="callee",
    ...
    if uses_type_edges:
        for target_type, rows in uses_type_edges.items():
            ...
            graph.add_connections(
                data=df,
                connection_type="USES_TYPE",
                source_type="Function",
                source_id_field="function",
                target_type=target_type,
                target_id_field="type_name",
            )
    ...
    if ffi_exposes_edges:
        ...
        for target_type, rows in by_target_type.items():
            ...
            graph.add_connections(
                data=df,
                connection_type="EXPOSES",
                source_type="Function",
                source_id_field="module_fn",
                target_type=target_type,
                target_id_field="target_qname",
            )
    ...
    if project_info is not None:
        if project_info.dependencies:
            ...
            graph.add_connections(
                ...
                source_type="Project",
                source_id_field="project",
                target_type="Dependency",
                target_id_field="dep_id",
            )
        if result.files:
            ...
            graph.add_connections(
                data=has_source_df,
                connection_type="HAS_SOURCE",
                source_type="Project",
                source_id_field="project",
                target_type="File",
                target_id_field="file_path",
            )
...
def _add_has_method_connections(graph, df, type_map):
    ...
    for src_nt, rows in groups.items():
        ...
        graph.add_connections(
            data=sub_df,
            connection_type="HAS_METHOD",
            source_type=src_nt,
            source_id_field="owner",
            target_type="Function",
            target_id_field="method",
        )
...
def _build_ffi_exposes_edges(result: ParseResult) -> list[dict]:
    ...
    pymodule_fns = [f for f in result.functions if f.metadata.get("is_pymodule")]
    if not pymodule_fns:
        return []
...
def build(
    ...
    *,
    ...
    if input_path.is_file():
        ...
        from .parsers.manifest import MANIFEST_READERS, ManifestReader
        ...
        if project_info is None:
            raise FileNotFoundError(f"Not a recognised manifest file: {input_path.name}")
    ...
    if project_info and project_info.source_roots:
        ...
        if verbose:
            root_labels = [str(r.path.relative_to(project_root)) for r in roots]
            print(f"Manifest: {project_info.manifest_path} ({project_info.build_system})")
    ...
    if verbose:
        print(
            f"Parsed: {len(result.functions)} functions, "
