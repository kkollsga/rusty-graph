...
class _BaseCCppParser(LanguageParser):
    ...
    def _has_storage_class(self, node, source: bytes, specifier: str) -> bool:
        for child in node.children:
                ...
                for field in child.children:
                    if field.type == "field_declaration":
                        ...
                        for fc in field.children:
                            ...
                            elif fc.type in ("pointer_declarator", "array_declarator"):
                                for inner in fc.children:
                                    if inner.type == "field_identifier":
                                        names.append(node_text(inner, source))
                                        break
                        for name in names:
    ...
    def _parse_struct(self, node, source: bytes, module_path: str, rel_path: str, result: ParseResult):
        ...
        result.attributes.extend(self._extract_struct_fields(node, source, qname, rel_path))
    ...
    def _parse_typedef(self, node, source: bytes, module_path: str, rel_path: str, result: ParseResult):
    ...
    def _parse_preproc_include(self, node, source: bytes, file_info: FileInfo):
    ...
    def _parse_preproc_def(self, node, source: bytes, module_path: str, rel_path: str, result: ParseResult):
    ...
    def parse_file(self, filepath: Path, src_root: Path) -> ParseResult:
        ...
        for child in root.children:
            if child.type == "function_definition":
                result.functions.append(self._parse_function(child, source, module_path, rel_path))

            elif child.type == "declaration":
                # Could be a function prototype, global var, or struct/enum
                has_struct = any(c.type == "struct_specifier" for c in child.children)
                has_enum = any(c.type == "enum_specifier" for c in child.children)
                if has_struct:
                    for sub in child.children:
                        if sub.type == "struct_specifier":
                            self._parse_struct(sub, source, module_path, rel_path, result)
                elif has_enum:
                    for sub in child.children:
                        if sub.type == "enum_specifier":
                            self._parse_enum(sub, source, module_path, rel_path, result)
                else:
                    # Global variable or function declaration
                    pass

            elif child.type == "type_definition":
                self._parse_typedef(child, source, module_path, rel_path, result)

            elif child.type == "preproc_include":
                self._parse_preproc_include(child, source, file_info)

            elif child.type == "preproc_def":
                self._parse_preproc_def(child, source, module_path, rel_path, result)
...
class CppParser(_BaseCCppParser):
    ...
    def _parse_class_specifier(
        self, node, source: bytes, module_path: str, rel_path: str, result: ParseResult, is_struct: bool = False
    ):
        ...
        self._parse_class_body(node, source, module_path, rel_path, name, qname, result, default_vis=default_vis)
            ...
            if child.type == "field_declaration_list":
                for item in child.children:
                    if item.type == "access_specifier":
                        ...
                        has_func_decl = any(c.type == "function_declarator" for c in item.children)
                        if has_func_decl:
                            ...
                            # Field declaration
                            self._parse_cpp_field(item, source, rel_path, class_qname, current_vis, result)
                    ...
                    elif item.type in ("class_specifier", "struct_specifier"):
                        # Nested class/struct
                        is_struct = item.type == "struct_specifier"
                        self._parse_class_specifier(
                            item, source, f"{module_path}::{class_name}", rel_path, result, is_struct=is_struct
                        )

        if method_type_rel.methods:
            result.type_relationships.append(method_type_rel)

    def _parse_cpp_field(
        self, node, source: bytes, rel_path: str, class_qname: str, visibility: str, result: ParseResult
    ):
    ...
    def _parse_namespace(
        self, node, source: bytes, parent_path: str, rel_path: str, result: ParseResult, file_info: FileInfo
    ):
        ...
        for child in node.children:
            if child.type == "declaration_list":
                for item in child.children:
                    self._parse_cpp_top_level(item, source, ns_path, rel_path, result, file_info)

    def _parse_cpp_top_level(
        self, node, source: bytes, module_path: str, rel_path: str, result: ParseResult, file_info: FileInfo
    ):
        """Parse a top-level C++ node."""
        if node.type == "function_definition":
            result.functions.append(self._parse_function(node, source, module_path, rel_path))

        elif node.type == "class_specifier":
            self._parse_class_specifier(node, source, module_path, rel_path, result, is_struct=False)

        elif node.type == "struct_specifier":
            self._parse_class_specifier(node, source, module_path, rel_path, result, is_struct=True)

        elif node.type == "declaration":
            # Check for class/struct/enum inside declarations
            for sub in node.children:
                if sub.type == "class_specifier":
                    self._parse_class_specifier(sub, source, module_path, rel_path, result, is_struct=False)
                    return
                elif sub.type == "struct_specifier":
                    self._parse_class_specifier(sub, source, module_path, rel_path, result, is_struct=True)
                    return
                elif sub.type == "enum_specifier":
                    self._parse_enum(sub, source, module_path, rel_path, result)
                    return
            # Check for function declaration
            has_func = any(c.type == "function_declarator" for c in node.children)
            if has_func:
                result.functions.append(self._parse_function(node, source, module_path, rel_path))
            ...
            self._parse_namespace(node, source, module_path, rel_path, result, file_info)
            ...
            for sub in node.children:
                if sub.type in ("function_definition", "class_specifier", "struct_specifier", "declaration"):
                    self._parse_cpp_top_level(sub, source, module_path, rel_path, result, file_info)
            ...
            self._parse_preproc_def(node, source, module_path, rel_path, result)
            ...
            for sub in node.children:
                if sub.type == "declaration_list":
                    for item in sub.children:
                        self._parse_cpp_top_level(item, source, module_path, rel_path, result, file_info)
    ...
    def parse_file(self, filepath: Path, src_root: Path) -> ParseResult:
        ...
        for child in root.children:
            self._parse_cpp_top_level(child, source, module_path, rel_path, result, file_info)
