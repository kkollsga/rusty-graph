...
class TestMapProjectionBasic:
    def test_property_shorthand(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.name, .age} AS info ORDER BY p.name")
        assert len(result) == 3
    ...
    def test_single_property(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.name} AS info ORDER BY p.name LIMIT 1")
    ...
    def test_system_properties(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.id, .name, .type} AS info ORDER BY p.name LIMIT 1")
    ...
    def test_null_property(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.name, .nonexistent} AS info ORDER BY p.name LIMIT 1")
        assert result[0]["info"]["name"] == "Alice"
...
class TestMapProjectionComputed:
    def test_alias_with_expression(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.name, label: p.type} AS info ORDER BY p.name LIMIT 1")
        assert result[0]["info"]["name"] == "Alice"
...
class TestMapProjectionIntegration:
    ...
    def test_without_alias(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.name} ORDER BY p.name LIMIT 1")
    ...
    def test_with_limit(self, social_graph):
        ...
        result = social_graph.cypher("MATCH (p:Person) RETURN p {.name, .age} AS info ORDER BY p.age LIMIT 2")
        assert len(result) == 2
