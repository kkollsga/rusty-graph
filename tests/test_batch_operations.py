...
class TestBatchUpdate:
    ...
    def test_update_with_filter(self):
        ...
        graph.add_nodes(df, "Node", "id", "name")
        result = graph.select("Node").where({"value": {">": 15}}).update({"processed": True})
...
class TestBulkOperations:
        ...
        nodes = [
            {
                "node_type": "Person",
                "id_field": "person_id",
                "data": pd.DataFrame({"person_id": [1, 2], "name": ["A", "B"], "age": [25, 30]}),
            },
            {
                "node_type": "Company",
                "id_field": "company_id",
                "data": pd.DataFrame({"company_id": [100], "name": ["Corp"]}),
            },
        ]
        for item in nodes:
            graph.add_nodes(item["data"], item["node_type"], item["id_field"], "name")
    ...
    def test_add_connections_bulk(self):
        ...
        report = graph.add_connections(conn, "LINKS", "Source", "source", "Target", "target")
