...
class TestValueRoundTrips:
    ...
    def test_integer_round_trip(self, graph):
        ...
        node = graph.type_filter('T').get_nodes()[0]
        assert node['val'] == 42
...
class TestFilterConditions:
    """Verify all filter operators via the .filter() API."""
    ...
    def test_equals(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'age': 30})
        assert result.node_count() == 1
        assert result.get_nodes()[0]['title'] == 'Bob'
    ...
    def test_greater_than(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'age': {'>': 35}})
    ...
    def test_in_list(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'age': {'==': [25, 45]}})
        assert result.node_count() == 2

    def test_in_operator(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'city': {'in': ['Oslo', 'Bergen']}})
        assert result.node_count() == 4

    def test_between(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'age': {'between': [30, 40]}})
        assert result.node_count() == 3

    def test_contains(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'city': {'contains': 'erg'}})
        assert result.node_count() == 2  # Bergen x2

    def test_starts_with(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'city': {'starts_with': 'Os'}})
        assert result.node_count() == 2

    def test_ends_with(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'city': {'ends_with': 'lo'}})
        assert result.node_count() == 2  # Oslo x2

    def test_is_null(self, populated_graph):
        """is_null finds nodes where property is missing."""
        result = populated_graph.type_filter('Person').filter({'email': {'is_null': True}})
        assert result.node_count() == 5  # no one has email

    def test_is_not_null(self, populated_graph):
        result = populated_graph.type_filter('Person').filter({'age': {'is_not_null': True}})
        assert result.node_count() == 5  # all have age

    def test_combined_filters(self, populated_graph):
        """Multiple filter conditions applied together (AND)."""
        result = populated_graph.type_filter('Person').filter({
            'age': {'>': 25},
            'city': 'Oslo',
        })
        assert result.node_count() == 1
        assert result.get_nodes()[0]['title'] == 'Charlie'
...
class TestBatchIngestion:
    ...
    def test_multiple_rows(self, graph):
        ...
        graph.add_nodes(df, 'T', 'id', 'name')
        assert graph.type_filter('T').node_count() == 100
