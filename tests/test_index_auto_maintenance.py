...
class TestMergeUpdatesIndex:
    ...
    def test_merge_create_path_indexed(self, indexed_graph):
        ...
        indexed_graph.cypher("MERGE (p:Person {name: 'Grace'}) ON CREATE SET p.city = 'Bodø', p.age = 29")
    ...
    def test_merge_match_path_set_updates_index(self, indexed_graph):
        ...
        indexed_graph.cypher("MERGE (p:Person {name: 'Alice'}) ON MATCH SET p.city = 'Drammen'")
...
class TestCompositeIndexMaintenance:
    ...
    def test_set_updates_composite_index(self):
        ...
        result = graph.cypher("MATCH (p:Person) WHERE p.city = 'Bergen' AND p.age = 28 RETURN p.name")
        names = [row["p.name"] for row in result]
        assert "Alice" in names

        # Verify old combo is gone — no one has Oslo+28 anymore
        result_old = graph.cypher("MATCH (p:Person) WHERE p.city = 'Oslo' AND p.age = 28 RETURN p.name")
