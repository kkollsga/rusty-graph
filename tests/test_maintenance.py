...
class TestVacuum:
    ...
    def test_vacuum_preserves_node_data(self, graph_with_data):
        ...
        result = graph_with_data.cypher("MATCH (p:Person) RETURN p.name ORDER BY p.name")
    ...
    def test_vacuum_then_create(self, graph_with_data):
        ...
        # Create a new node
        graph_with_data.cypher("CREATE (p:Person {name: 'NewPerson', age: 99})")
        assert graph_with_data.last_mutation_stats["nodes_created"] == 1

        # Verify the new node exists
        result = graph_with_data.cypher("MATCH (p:Person {name: 'NewPerson'}) RETURN p.age")
        assert len(result) == 1
