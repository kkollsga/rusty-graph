"""Tests for calculate, statistics, count, collect_children, connection aggregation."""
...
class TestCalculate:
    def test_simple_expression(self):
        ...
        result = graph.select('Product').calculate(
    ...
    def test_aggregate_sum(self):
        ...
        result = graph.select('Parent').traverse('HAS_CHILD').calculate(
...
class TestCount:
    def test_count_basic(self):
        ...
        plays = graph.select('Parent')
        assert plays.len() == 2
        children_sel = plays.traverse('HAS')
        assert children_sel.len() == 3


class TestConnectionAggregation:
    def test_sum_connection_properties(self, petroleum_graph):
        result = petroleum_graph.select('Prospect').traverse('BECAME_DISCOVERY').calculate(
    ...
    def test_avg_connection_properties(self, petroleum_graph):
        result = petroleum_graph.select('Prospect').traverse('HAS_ESTIMATE').calculate(
...
class TestChildrenPropertiesToList:
    def test_basic(self):
        ...
        result = graph.select('Parent').traverse('HAS').collect_children(
            property='title',
        )
        assert result is not None
