...
class TestPatternMatchPerformance:
    def test_pattern_match_scale(self):
        ...
        graph.add_connections(pd.DataFrame(edges), "KNOWS", "Person", "from_id", "Person", "to_id")
...
def _build_cypher_bench_graph(n_people=100, n_companies=20):
    ...
    graph.add_connections(pd.DataFrame(knows), "KNOWS", "Person", "from_id", "Person", "to_id")
    ...
    graph.add_connections(works, "WORKS_AT", "Person", "person_id", "Company", "company_id")
...
class TestCypherPerformance:
    ...
    def test_where_filter(self):
        ...
        graph = _build_cypher_bench_graph(500)
        start = time.time()
        result = graph.cypher("MATCH (n:Person) WHERE n.age > 40 RETURN n.title, n.age")
        elapsed = time.time() - start
        assert len(result) > 0
    ...
    def test_edge_traversal(self):
        ...
        start = time.time()
        result = graph.cypher("MATCH (a:Person)-[:KNOWS]->(b:Person) RETURN a.title, b.title")
        elapsed = time.time() - start
        assert len(result) > 0
    ...
    def test_cross_type_join(self):
        ...
        start = time.time()
        result = graph.cypher("MATCH (p:Person)-[:WORKS_AT]->(c:Company) RETURN p.title, c.title")
        elapsed = time.time() - start
        assert len(result) == 500
    ...
    def test_aggregation(self):
        ...
        graph = _build_cypher_bench_graph(500)
        start = time.time()
        result = graph.cypher("MATCH (n:Person) RETURN n.city AS city, count(*) AS cnt, avg(n.salary) AS avg_salary")
        elapsed = time.time() - start
        assert len(result) > 0
    ...
    def test_order_limit(self):
        ...
        graph = _build_cypher_bench_graph(500)
        start = time.time()
        result = graph.cypher("MATCH (n:Person) RETURN n.title, n.salary ORDER BY n.salary DESC LIMIT 10")
        elapsed = time.time() - start
    ...
    def test_cypher_vs_fluent_api(self):
        ...
        # Cypher approach
        start = time.time()
        cypher_result = graph.cypher("MATCH (n:Person) WHERE n.age > 40 RETURN n.title")
        ...
        print(
            f"  Cypher: {cypher_time * 1000:.1f}ms vs Fluent API: {fluent_time * 1000:.1f}ms "
    ...
    def test_cypher_indexed_vs_unindexed(self):
        ...
        print(
            f"  Cypher WHERE no index: {no_idx_time * 1000:.1f}ms, "
    ...
    def test_cypher_scaling(self, n):
        ...
        start = time.time()
        result = graph.cypher("MATCH (a:Person)-[:KNOWS]->(b:Person) WHERE a.age > 30 RETURN a.title, b.title")
        elapsed = time.time() - start
        assert len(result) > 0
...
class TestCypherMutationPerformance:
    ...
    def test_delete_relationship_only(self):
        ...
        print(
            f"  Delete relationships for 1 node: {elapsed * 1000:.2f}ms, "
    ...
    def test_detach_delete_all_scaling(self, n):
        ...
        print(
            f"  Detach delete all {n} people: {elapsed * 1000:.1f}ms, {result['stats']['relationships_deleted']} edges"
        )
    ...
    def test_delete_subset(self):
        ...
        print(
            f"  Delete subset (age>60): {elapsed * 1000:.1f}ms, "
    ...
    def test_remove_scaling(self, n):
        ...
        print(
            f"  Remove 2 props from {n} nodes: {elapsed * 1000:.1f}ms, {result['stats']['properties_removed']} removed"
        )
    ...
    def test_merge_creates_single_node(self):
        ...
        graph = _build_cypher_bench_graph(500)
        start = time.time()
        result = graph.cypher("MERGE (n:Person {name: 'NewPerson'}) ON CREATE SET n.age = 99")
        elapsed = time.time() - start
    ...
    def test_merge_matches_existing_node(self):
        ...
        graph = _build_cypher_bench_graph(500)
        start = time.time()
        result = graph.cypher("MERGE (n:Person {name: 'Person_0'}) ON MATCH SET n.visits = 1")
        elapsed = time.time() - start
    ...
    def test_merge_match_scaling(self, n):
        ...
        start = time.time()
        result = graph.cypher(f"MERGE (n:Person {{name: '{target}'}}) ON MATCH SET n.visits = 1")
        elapsed = time.time() - start
        assert result["stats"]["nodes_created"] == 0
    ...
    def test_merge_batch_create_nodes(self):
        ...
        start = time.time()
        for i in range(100):
            graph.cypher(f"MERGE (n:City {{name: 'City_{i}'}}) ON CREATE SET n.pop = {i * 1000}")
        elapsed = time.time() - start
        # Verify all created
        check = graph.cypher("MATCH (n:City) RETURN count(*) AS cnt")
        assert check[0]["cnt"] == 100
        print(f"  Merge-create 100 cities (sequential): {elapsed * 1000:.1f}ms ({elapsed * 10:.2f}ms/op)")
    ...
    def test_merge_batch_match_nodes(self):
        ...
        start = time.time()
        for i in range(100):
            graph.cypher(f"MERGE (n:Person {{name: 'Person_{i}'}}) ON MATCH SET n.visited = 1")
        elapsed = time.time() - start
        print(f"  Merge-match 100 of 500 people (sequential): {elapsed * 1000:.1f}ms ({elapsed * 10:.2f}ms/op)")
...
class TestCypherMutationCombined:
    ...
    def test_merge_then_remove(self):
        ...
        start = time.time()
        graph.cypher("MERGE (n:Person {name: 'Person_0'}) ON MATCH SET n.temp_flag = 'processing'")
    ...
    def test_full_mutation_pipeline(self, n):
        ...
        print(
            f"  Pipeline {n} nodes: delete {deleted} edges={delete_time * 1000:.1f}ms, "
